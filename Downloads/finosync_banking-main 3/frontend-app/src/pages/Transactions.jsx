        ...
        <tbody>
          {filtered.map((t) => (
            <tr key={t._id || t.id}>
              ...
              <td>{t.category}</td>
              <td>{t.type}</td>
              <td className={["DEBIT", "EMI"].includes(t.type?.trim().toUpperCase()) ? "amount-negative" : "amount-positive"}>
                {["DEBIT", "EMI"].includes(t.type?.trim().toUpperCase()) ? "-" : "+"}₹{Number(t.amount).toFixed(2)}
              </td>
            </tr>
          ))}
        </tbody>
