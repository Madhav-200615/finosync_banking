  ...
  const handleRequestCard = async (e) => {
    ...
    try {
      ...
      if (error.response?.status === 400) {
        alert(error.response?.data?.error || "Invalid card details. Please check your input.");
      }
      // 401 errors are handled by the global interceptor
      else if (error.response?.status === 401) {
        // This will be caught by the interceptor, but we can add a fallback
        alert("Authentication failed. Please log in again.");
      }
      // Other errors
      else {
        alert(error.response?.data?.error || "Failed to request card. Please try again.");
      }
    }
  };
